    ...
    <Layout>
          ...
          <Grid container spacing={3}>
            <PlanDisplay
              onPlanClick={(priceItem) => {
                const selectedPlan = plans.find(plan => plan.PlanTypePrices.find(price => price.id === priceItem.id))
                setSelectedPlan({
                  price: priceItem, plan: {
                    id: selectedPlan.id,
      ...
      {selectedPlan && (
        <>
          ...
          <DialogContent>
            <DialogContentText style={{ margin: 20 }} id="alert-dialog-slide-description">
          ...
          <DialogActions style={{margin: 20}}>
            ...
            <Buttons variant='outlined' color="secondary" onClick={() => handlePlanChoose(selectedPlan.price, { anotherCard: true })}>Procced With Another Card</Buttons>
            <Buttons variant='outlined' color="secondary" onClick={() => handlePlanChoose(selectedPlan.price, { anotherCard: false })}>Proceed</Buttons>
          </DialogActions>
        </>
      )}
      </Dialog>
