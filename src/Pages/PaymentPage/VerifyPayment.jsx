...
export default function VerifyPayment() {
    const [paymentStatus, setPaymentStatus] = useState(PAYMENT_STATUS_STATE.LOADING);

    const updatePaymentStatusIfPaid = async () => {
        try {
            const sessionId = new URLSearchParams(window.location.search).get('session_id');
            const response = await axios.post(`/payment/session`, { token: sessionId });
            setPaymentStatus(response.data.data.message === 'Payment success' ? PAYMENT_STATUS_STATE.PAID : PAYMENT_STATUS_STATE.NOT_PAID);
    ...
    if (paymentStatus === PAYMENT_STATUS_STATE.LOADING) {
        return (
            <Alert severity="info">Fetching your payment success status</Alert>
        )
    }
    ...
    if (paymentStatus === PAYMENT_STATUS_STATE.EXPIRED) {
        return (
            <Alert severity="warning">Could not find your detail.</Alert>
        )
    }
